...
use Carbon\Carbon;
use App\Booking;
class PerformencedController extends Controller
{
      ...
      for($i = 0;$i<7; $i++)
        {
          ...
          foreach($bookings as $booking)
            {
              if(Carbon::parse($booking->date)->format('Y-m-d') == $str_week_start && $booking->status == "approved")
                {
                  $disp_week_data[$i]++;
                }         
            }
        ...
        for($j=0;$j<$lastday;$j++)
        {
          ...
          foreach($bookings as $booking)
          {
            if(Carbon::parse($booking->adate)->format('Y-m-d') == $str_month_start && $booking->status == "approved")
            {
               $disp_month_data[$j]++;
            }         
          }
        ...
        for($k=0;$k<12;$k++)
          {
            ...
            for($u=0;$u<$lastday;$u++)
            {
              foreach($bookings as $booking)
              {
                if(Carbon::parse($booking->date)->format('Y-m-d') == $str_year_month && $booking->status == "approved")
                {
                  $disp_year_month[$k]++;
                }         
              }               
         ...
         return view('admin.reports.perfomed_services',compact('disp_year_month','disp_week_data','disp_month_data','year_month','week_data','month_data','currentMonthName','currentYear','currentTime'));
      }



   public function getPrevPerformedServices(Request $request)
    {
      $disp_month_data = [];
      $disp_year_month = [];
      $disp_week_data = [];
      ...
      for($i = 0;$i<7; $i++)
          ...
          foreach($bookings as $booking)
            {
              if(Carbon::parse($booking->date)->format('Y-m-d') == $str_week_start && $booking->status == "approved")
                {
                  $disp_week_data[$i]+=(int)substr($booking->price,0,-1);
                }         
            }
                $week_start->addDay(1);
                array_push($week_data,$str_week_start);
        ...
        for($j=0;$j<$lastday;$j++)
        {
          ...
          foreach($bookings as $booking)
          {
            if(Carbon::parse($booking->date)->format('Y-m-d') == $str_month_start && $booking->status == "approved")
            {
               $disp_month_data[$j]+=(int)substr($booking->price,0,-1);
            }         
          }
            $month_start->addDay(1);
            array_push($month_data,$str_month_start);
        ...
        for($k=0;$k<12;$k++)
          {
            ...
            for($u=0;$u<$lastday;$u++)
            {
              foreach($bookings as $booking)
              {
                if(Carbon::parse($booking->date)->format('Y-m-d') == $str_year_month && $booking->status == "approved")
                {
                  $disp_year_month[$k]+=(int)substr($booking->price,0,-1);
                }         
              }               
                $year_start->addDay(1);                    
                $str_year_month = $year_start->format('Y-m-d');            
    ...
    public function getNextPerformedServices(Request $request)
    {
      $disp_month_data = [];
      $disp_year_month = [];
      $disp_week_data = [];
